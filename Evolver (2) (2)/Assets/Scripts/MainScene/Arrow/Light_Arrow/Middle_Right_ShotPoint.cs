...
public class Middle_Right_ShotPoint : MonoBehaviour
{
    ...
    float launchForce; float increaseDamage; float increaseLaunchForce; bool ischanged; float chargingDamage;   float TempDMG;  float TempLF; float TempDegree;
    ...
    public void Shoot()
    {
        ...
        if (Random.Range(0, 100) < Player_Stat.instance.criticalPercent || Player_Stat.instance.AbsolCrit || Player_Stat.instance.Runner)
        {
            ...
            if (SPCache.GetComponent<ShotPoint>().tester)
            {
    ...
    IEnumerator HoamingDelay()
    {
        ...
        temp = Instantiate(HoamingArrow, this.transform.position, this.transform.rotation);
        temp.GetComponent<Hoaming_Arrow>().Launched = true;
    }
}
